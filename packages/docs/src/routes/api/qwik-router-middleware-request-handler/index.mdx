...
<h2 id="clearloadercache">clearLoaderCache</h2>
...
<h2 id="requestevloaderfilteredsearch">RequestEvLoaderFilteredSearch</h2>

SharedMap key containing the route-loader request search string after applying the loader's `search` allowlist. Available during route-loader `cacheKey` callbacks.

```typescript
RequestEvLoaderFilteredSearch = "@loaderFilteredSearch";
```

[Edit this section](https://github.com/QwikDev/qwik/tree/main/packages/qwik-router/src/middleware/request-handler/request-event-core.ts)
