...
Loader `cacheKey` callbacks can read the filtered search string from `ev.sharedMap.get(RequestEvLoaderFilteredSearch)`, using the exported key from `@qwik.dev/router/middleware/request-handler`.
