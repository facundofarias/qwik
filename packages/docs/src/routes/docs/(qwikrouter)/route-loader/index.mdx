...
When a loader `cacheKey` callback needs the loader-filtered search string, read `ev.sharedMap.get(RequestEvLoaderFilteredSearch)` using the exported key from `@qwik.dev/router/middleware/request-handler`.
